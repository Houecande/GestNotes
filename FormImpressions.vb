...
Public Class FormImpressions
    ...
    Private Sub FormImpressions_Load(sender As Object, e As EventArgs) _
        ...
        ' Configuration des périodes (Semestres) selon la structure de la table Bulletin
        cmbPeriode.Items.Clear()
        cmbPeriode.Items.Add("Semestre 1")
    ...
    Private Function PageAccueil() As String
        Return "<!DOCTYPE html><html><head>" &
               ...
               "<div class='icon'>&#128424;</div>" &
               "<h2>Impressions</h2>" &
               "<p>Sélectionnez une classe et une période, puis cliquez sur Aperçu</p>" &
               "</div></body></html>"
    End Function
    ...
    Private Sub cmbTypeDoc_SelectedIndexChanged(sender As Object, e As EventArgs) _
    ...
    Private Sub cmbClasseImp_SelectedIndexChanged(sender As Object, e As EventArgs) _
    ...
    Private Sub btnApercu_Click(sender As Object, e As EventArgs) _
        ...
        If cmbClasseImp.SelectedIndex < 0 Then
            MsgBox("Veuillez sélectionner une classe.", MsgBoxStyle.Exclamation, "Classe requise")
            Exit Sub
        End If
    ...
    Private Sub btnImprimer_Click(sender As Object, e As EventArgs) _
        ...
        If wbApercu.Document Is Nothing OrElse String.IsNullOrEmpty(wbApercu.DocumentText) Then
            MsgBox("Générez d'abord un aperçu.", MsgBoxStyle.Exclamation, "Aperçu requis")
            Exit Sub
        End If
    ...
    Private Sub btnExporterPDF_Click(sender As Object, e As EventArgs) _
        ...
        Dim periodeText = If(cmbTypeDoc.SelectedIndex = 0, cmbPeriode.SelectedItem.ToString().Replace(" ", ""), "")
        sfd.FileName = If(cmbTypeDoc.SelectedIndex = 0,
            $"Bulletin_{periodeText}_{DateTime.Now:yyyyMMdd}",
            $"Releve_{DateTime.Now:yyyyMMdd}")
        ...
        Try
            Dim html = If(cmbTypeDoc.SelectedIndex = 0, GenererHTMLBulletin(), GenererHTMLReleve())
            Dim tempHtml = Path.Combine(Path.GetTempPath(), "gn_export_temp.html")
            ...
            If cmbTypeDoc.SelectedIndex = 0 Then
                ...
                For Each row As DataRow In dtEtudiants.Rows
                    ...
                    For Each nr As DataRow In dtN.Rows
                        ...
                        Dim m As Double = 0
                        Double.TryParse(nr("moyenne_matiere").ToString().Replace(",", "."),
    ...
    Private Function GetCSS() As String
        Return "<style>" &
            ...
            ".bulletin { background: white; max-width: 750px; margin: 0 auto 30px; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,0.1); }" &
            ".header { background: #0C447C; color: white; padding: 20px; text-align: center; }" &
            ".header h1 { font-size: 16px; font-weight: 600; letter-spacing: 1px; margin-bottom: 4px; }" &
            ".header p { font-size: 11px; opacity: 0.8; }" &
            ".doc-title { background: #185FA5; color: white; text-align: center; padding: 10px; font-size: 14px; font-weight: 600; letter-spacing: 2px; }" &
            ".doc-annee { background: #E6F1FB; color: #0C447C; text-align: center; padding: 6px; font-size: 12px; }" &
            ".info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0; border-bottom: 1px solid #E6F1FB; }" &
            ".info-item { padding: 8px 16px; font-size: 12px; color: #333; border-bottom: 1px solid #F0F4F8; }" &
            ".info-item span { color: #185FA5; font-weight: 600; margin-right: 6px; }" &
            ...
            ".moy-cell { font-weight: 700; border-radius: 4px; padding: 3px 8px; display: inline-block; }" &
            ...
            ".resume { background: #E6F1FB; border-left: 4px solid #185FA5; padding: 14px 16px; margin: 0 16px 16px; border-radius: 0 6px 6px 0; display: flex; justify-content: space-between; align-items: center; }" &
            ".resume-item { text-align: center; }" &
            ".resume-label { font-size: 10px; color: #888; text-transform: uppercase; letter-spacing: 1px; }" &
            ".resume-value { font-size: 18px; font-weight: 700; color: #0C447C; }" &
            ".decision-admis { background: #059669; color: white; padding: 8px 20px; border-radius: 20px; font-weight: 600; font-size: 13px; }" &
            ".decision-ajourn { background: #DC2626; color: white; padding: 8px 20px; border-radius: 20px; font-weight: 600; font-size: 13px; }" &
            ".footer { border-top: 1px solid #E6F1FB; padding: 14px 16px; display: flex; justify-content: space-between; font-size: 11px; color: #888; }" &
    ...
    Private Function GenererHTMLBulletin() As String
        Dim classe = cmbClasseImp.SelectedItem.ToString()
        Dim annee = txtAnneeScolaire.Text
        Dim periode = cmbPeriode.SelectedItem.ToString()

        Dim dtParam = ModuleBDD.GetDataTable("SELECT nom_institut, adresse FROM Parametres")
        Dim nomInstitut = If(dtParam.Rows.Count > 0, dtParam.Rows(0)("nom_institut").ToString(), "INSTITUT")
        Dim adresse = If(dtParam.Rows.Count > 0, dtParam.Rows(0)("adresse").ToString(), "")
        ...
        For idx As Integer = 0 To dtEtudiants.Rows.Count - 1
            ...
            Dim dateNaiss = "—"
            Try
                dateNaiss = CDate(etudRow("date_naissance")).ToString("dd/MM/yyyy")
            Catch : End Try
            ...
            For Each nr As DataRow In dtNotes.Rows
                ...
                Double.TryParse(nr("moyenne_matiere").ToString().Replace(",", "."),
                                System.Globalization.NumberStyles.Any,
                                System.Globalization.CultureInfo.InvariantCulture, moy)
                totalPts += moy * coef
                totalCoefs += coef
            Next

            Dim moyGen As Double = If(totalCoefs > 0, Math.Round(totalPts / totalCoefs, 2), 0)
            Dim rang = CalculerRang(matricule, classe)
            Dim estAdmis = moyGen >= 10
            Dim isLast = (idx = dtEtudiants.Rows.Count - 1)

            sb.AppendLine("<div class='bulletin" & If(Not isLast, " page-break", "") & "'>")
            sb.AppendLine("<div class='header'><h1>" & HtmlEncode(nomInstitut) & "</h1><p>" & HtmlEncode(adresse) & "</p></div>")
            sb.AppendLine($"<div class='doc-title'>BULLETIN DE NOTES ({periode.ToUpper()})</div>")
            sb.AppendLine("<div class='doc-annee'>Année scolaire : " & annee & "</div>")
            sb.AppendLine("<div class='info-grid'>")
            sb.AppendLine("<div class='info-item'><span>Matricule :</span>" & HtmlEncode(matricule) & "</div>")
            sb.AppendLine("<div class='info-item'><span>Classe :</span>" & HtmlEncode(classe) & "</div>")
            sb.AppendLine("<div class='info-item'><span>Nom &amp; Prénom :</span>" & HtmlEncode(nom) & "</div>")
            sb.AppendLine("<div class='info-item'><span>Sexe :</span>" & HtmlEncode(sexe) & "</div>")
            sb.AppendLine("<div class='info-item'><span>Date de naissance :</span>" & dateNaiss & "</div>")
            sb.AppendLine("<div class='info-item'><span>Lieu de naissance :</span>" & HtmlEncode(lieuNaiss) & "</div>")
            sb.AppendLine("</div>")

            sb.AppendLine("<div class='section'><table><thead><tr>" &
                          "<th>Matière</th><th>Coef.</th><th>Interrogation</th><th>Devoir</th><th>Moyenne</th><th>Appréciation</th>" &
                          "</tr></thead><tbody>")

            For Each nr As DataRow In dtNotes.Rows
                Dim matiere = nr("libelle_matiere").ToString()
                Dim coef = nr("coefficient").ToString()
                Dim interro As Double = 0 : Dim devoir As Double = 0 : Dim moy As Double = 0
                Double.TryParse(nr("note_interro").ToString().Replace(",", "."), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, interro)
                Double.TryParse(nr("note_devoir").ToString().Replace(",", "."), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, devoir)
                Double.TryParse(nr("moyenne_matiere").ToString().Replace(",", "."), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, moy)

                sb.AppendLine("<tr>")
                sb.AppendLine("<td>" & HtmlEncode(matiere) & "</td>")
                sb.AppendLine("<td style='text-align:center'>" & coef & "</td>")
                sb.AppendLine("<td style='text-align:center'>" & interro.ToString("F2") & "</td>")
                sb.AppendLine("<td style='text-align:center'>" & devoir.ToString("F2") & "</td>")
                sb.AppendLine("<td style='text-align:center'><span class='moy-cell " & GetMoyCSS(moy) & "'>" & moy.ToString("F2") & "</span></td>")
            ...
            sb.AppendLine("<div class='resume-item'><div class='resume-label'>Moyenne générale</div><div class='resume-value'>" & moyGen.ToString("F2") & "/20</div></div>")
            sb.AppendLine("<div class='resume-item'><div class='resume-label'>Rang</div><div class='resume-value'>" & rang & "</div></div>")
            sb.AppendLine("<div class='resume-item'>")
            sb.AppendLine(If(estAdmis, "<span class='decision-admis'>&#10003; ADMIS(E)</span>", "<span class='decision-ajourn'>&#10007; AJOURNÉ(E)</span>"))
            sb.AppendLine("</div></div>")
            sb.AppendLine("<div class='footer'><span>Édité le : " & DateTime.Now.ToString("dd/MM/yyyy") & "</span>")
            sb.AppendLine("<div style='text-align:center'><div class='sig-line'></div><div>Le Directeur</div></div></div></div>")
        Next
    ...
    Private Function GenererHTMLReleve() As String
        ...
        For Each row As DataRow In dtEtudiants.Rows
            ...
            For Each nr As DataRow In dtN.Rows
                Dim c = CInt(nr("coefficient"))
                Dim m As Double = 0
                Double.TryParse(nr("moyenne_matiere").ToString().Replace(",", "."), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, m)
                pts += m * c : coefs += c
            Next
            Dim mg As Double = If(coefs > 0, Math.Round(pts / coefs, 2), 0)
        ...
        Dim moyC As Double = If(trie.Count > 0, Math.Round(trie.Average(Function(x) x.Item3), 2), 0)

        Dim sb As New System.Text.StringBuilder()
        sb.AppendLine("<!DOCTYPE html><html><head><meta charset='utf-8'>")
        sb.AppendLine(GetCSS())
        sb.AppendLine("</head><body><div class='bulletin'><div class='header'><h1>" & HtmlEncode(nomInstitut) & "</h1></div>")
        sb.AppendLine("<div class='doc-title'>RELEVÉ DE NOTES — PAR ORDRE DE MÉRITE</div>")
        sb.AppendLine("<div class='doc-annee'>Classe : " & classe & " &nbsp;|&nbsp; Année : " & annee & "</div>")
        sb.AppendLine("<div class='section'><table><thead><tr><th>Rang</th><th>Matricule</th><th>Nom &amp; Prénom</th><th>Moy. Générale</th><th>Décision</th></tr></thead><tbody>")

        Dim rang As Integer = 1
        For Each etud In trie
            Dim dec = If(etud.Item3 >= 10, "<span class='decision-admis' style='padding:3px 10px;font-size:11px'>ADMIS</span>", "<span class='decision-ajourn' style='padding:3px 10px;font-size:11px'>AJOURNÉ</span>")
            sb.AppendLine("<tr>")
            sb.AppendLine("<td style='text-align:center;font-weight:700;color:#185FA5'>" & rang & GetSuffixeRang(rang) & "</td>")
            sb.AppendLine("<td>" & HtmlEncode(etud.Item1) & "</td>")
            sb.AppendLine("<td><b>" & HtmlEncode(etud.Item2) & "</b></td>")
            sb.AppendLine("<td style='text-align:center'><span class='moy-cell " & GetMoyCSS(etud.Item3) & "'>" & etud.Item3.ToString("F2") & "</span></td>")
        ...
        sb.AppendLine("</tbody></table></div>")
        sb.AppendLine("<div class='resume'>")
        sb.AppendLine("<div class='resume-item'><div class='resume-label'>Effectif</div><div class='resume-value'>" & trie.Count & "</div></div>")
        sb.AppendLine("<div class='resume-item'><div class='resume-label'>Admis</div><div class='resume-value' style='color:#059669'>" & nbAdmis & "</div></div>")
        sb.AppendLine("<div class='resume-item'><div class='resume-label'>Ajournés</div><div class='resume-value' style='color:#DC2626'>" & nbAj & "</div></div>")
        sb.AppendLine("<div class='resume-item'><div class='resume-label'>Moy. classe</div><div class='resume-value'>" & moyC.ToString("F2") & "</div></div>")
        sb.AppendLine("<div class='resume-item'><div class='resume-label'>Taux réussite</div><div class='resume-value'>" & If(trie.Count > 0, Math.Round(nbAdmis * 100.0 / trie.Count, 0) & "%", "—") & "</div></div>")
        sb.AppendLine("</div><div class='footer'><span>Édité le : " & DateTime.Now.ToString("dd/MM/yyyy HH:mm") & "</span>")
        sb.AppendLine("<div style='text-align:center'><div class='sig-line'></div><div>Le Directeur</div></div></div></div></body></html>")
        Return sb.ToString()
    End Function
    ...
    Private Function CalculerRang(matricule As String, classe As String) As String
        Try
            ...
            Dim trie = moyennes.OrderByDescending(Function(x) x.Item2).ToList()
            Dim rang As Integer = 1
            For Each item In trie
                If item.Item1 = matricule Then Return rang & GetSuffixeRang(rang) & " / " & trie.Count
                rang += 1
            Next
            Return "— / " & trie.Count
        Catch
