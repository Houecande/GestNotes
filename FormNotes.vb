...
Public Class FormNotes
    ...
    Private Sub cmbMatiere_SelectedIndexChanged(
        ...
        ' Extraire le code matière (avant le " — ")
    ...
    ' Charger les étudiants + leurs notes existantes
    Private Sub ChargerNotesEtudiants()
        dgvNotes.Rows.Clear()

        ' Récupérer tous les étudiants de la classe
        Dim dtEtudiants = ModuleBDD.GetDataTable(
            "SELECT num_matricule, nom_prenom FROM Etudiant " &
        ...
        For Each row As DataRow In dtEtudiants.Rows
            ...
            ' Chercher les notes existantes pour cet étudiant/matière
            Dim dtNote = ModuleBDD.GetDataTable(
                "SELECT note_interro, note_devoir, moyenne_matiere " &
                "FROM Note WHERE num_matricule = ? " &
                "AND code_matiere = ?",
                New OleDbParameter("@mat", matricule),
                New OleDbParameter("@code", CodeMatiereSelectionne))

            If dtNote.Rows.Count > 0 Then
                ' Notes existantes → les afficher
                Dim noteRow = dtNote.Rows(0)
                Dim interro = noteRow("note_interro").ToString()
                Dim devoir = noteRow("note_devoir").ToString()
                Dim moy = noteRow("moyenne_matiere").ToString()
                Dim rowIdx = dgvNotes.Rows.Add(
                    num, nom, interro, devoir, moy)
                ' Coloration de la moyenne
                ColorierMoyenne(rowIdx,
                    If(moy <> "", CDbl(moy), -1))
            Else
                ' Pas encore de notes
        ...
        ' Stocker les matricules dans la grille (colonne cachée)
        ' On utilise le Tag de la grille pour stocker la liste
        Dim matricules As New List(Of String)
        For Each row As DataRow In dtEtudiants.Rows
            matricules.Add(row("num_matricule").ToString())
        Next
    ...
    ' Édition d'une cellule (note saisie) 
    Private Sub dgvNotes_CellEndEdit(sender As Object,
        ...
        Dim valeur = dgvNotes.Rows(row).Cells(col).Value?.ToString().Trim()

        ' Validation
        Dim note As Double = -1
        If Not String.IsNullOrEmpty(valeur) Then
            If Not Double.TryParse(valeur.Replace(",", "."),
                                   note) OrElse
               note < 0 OrElse note > 20 Then
                MsgBox("La note doit être un nombre entre 0 et 20.",
        ...
        ' Calcul automatique si les deux notes sont saisies
        Dim valInterro = dgvNotes.Rows(row).Cells(2).Value?.ToString().Trim()
        Dim valDevoir = dgvNotes.Rows(row).Cells(3).Value?.ToString().Trim()
        ...
        If Not String.IsNullOrEmpty(valInterro) AndAlso
           Not String.IsNullOrEmpty(valDevoir) Then
            ...
            If Double.TryParse(valInterro.Replace(",", "."), interro) AndAlso
               Double.TryParse(valDevoir.Replace(",", "."), devoir) Then
                Dim moy = Math.Round(0.3 * interro + 0.7 * devoir, 2)
                dgvNotes.Rows(row).Cells(4).Value = moy.ToString("F2")
    ...
    ' Colorer la cellule Moyenne selon la valeur 
    ...
    ' Bouton Calculer (recalcul global) 
    Private Sub btnCalculer_Click(sender As Object, e As EventArgs) _
        Handles btnCalculer.Click
        Dim nbCalcules As Integer = 0

        For i As Integer = 0 To dgvNotes.Rows.Count - 1
            Dim valInterro = dgvNotes.Rows(i).Cells(2).Value?.ToString().Trim()
            Dim valDevoir = dgvNotes.Rows(i).Cells(3).Value?.ToString().Trim()

            If Not String.IsNullOrEmpty(valInterro) AndAlso
               Not String.IsNullOrEmpty(valDevoir) Then
                Dim interro As Double
                Dim devoir As Double
                If Double.TryParse(valInterro.Replace(",", "."),
                                   interro) AndAlso
                   Double.TryParse(valDevoir.Replace(",", "."),
                                   devoir) Then
                    Dim moy = Math.Round(0.3 * interro + 0.7 * devoir, 2)
    ...
    Private Sub btnEnregistrer_Click(sender As Object, e As EventArgs) _
        ...
        For i As Integer = 0 To dgvNotes.Rows.Count - 1
            ...
            If String.IsNullOrEmpty(valInterro) AndAlso
               String.IsNullOrEmpty(valDevoir) Then Continue For

            Dim interro As Double = 0
            Dim devoir As Double = 0
            If Not String.IsNullOrEmpty(valInterro) Then
                Double.TryParse(valInterro.Replace(",", "."), interro)
            End If
            If Not String.IsNullOrEmpty(valDevoir) Then
                Double.TryParse(valDevoir.Replace(",", "."), devoir)
            End If

            Dim moy As Double = 0
            Dim valMoy = dgvNotes.Rows(i).Cells(4).Value?.ToString()
            If Not String.IsNullOrEmpty(valMoy) AndAlso valMoy <> "—" Then
                Double.TryParse(valMoy.Replace(",", "."), moy)
            End If
        ...
        ' Message résultat
        If nbErreurs = 0 Then
            MsgBox(nbEnregistres & " note(s) enregistrée(s) avec succès." &
                   vbCrLf & "Classe : " & ClasseSelectionnee &
                   vbCrLf & "Matière : " & CodeMatiereSelectionne,
                   MsgBoxStyle.Information, "Enregistrement réussi")
        Else
            MsgBox(nbEnregistres & " note(s) enregistrée(s)." & vbCrLf &
                   nbErreurs & " erreur(s) rencontrée(s).",
                   MsgBoxStyle.Exclamation, "Enregistrement partiel")
        End If
    ...
    Private Sub MettreAJourStats()
        ...
        For Each row As DataGridViewRow In dgvNotes.Rows
            ...
            If Not String.IsNullOrEmpty(valMoy) AndAlso
               valMoy <> "—" Then
                Dim moy As Double
                If Double.TryParse(valMoy.Replace(",", "."), moy) Then
                    moyennes.Add(moy)
                    nbSaisies += 1
                End If
            End If
        ...
        lblValMoyClasse.Text =
            Math.Round(moyennes.Average(), 2).ToString("F2")
